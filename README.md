...
- `POST /api/auth/login`
- `GET /api/auth/me`
- `GET /api/integrations/iiko/check`
- `GET /api/integrations/iiko/organizations`
- `GET /api/integrations/iiko/terminal-groups`
- `GET /api/integrations/iiko/nomenclature`
- `GET /api/integrations/iiko/overview`
...
После деплоя можно проверить интеграцию запросом к endpoint `GET /api/integrations/iiko/check`.
...
3. Нажмите Deploy.
4. Проверьте `GET /api/integrations/iiko/check` — должен вернуть `connection_status: success`.
...
```bash
curl -X GET "https://<ваш-домен>/api/integrations/iiko/check"
```
...
Если внутренняя сеть платформы изолирована, задайте `BACKEND_PUBLIC_URL` (внешний URL backend-сервиса). Это приоритетный target для прокси.


### iiko в интерфейсе
Откройте `/iiko` в существующей панели. Страница показывает:
- Connection status
- Token received
- Organizations found
- Selected organization
- Terminal groups found
- Products loaded
- Revision

Если `IIKO_ORGANIZATION_ID` не задан, можно выбрать организацию прямо в UI.
